    ...
    namespace coder
    {
      namespace qpactiveset
      {
                     ...
                     options_ObjectiveLimit, int runTimeOptions_MaxIterations)
        {
          ...
          std::memset(&solution->lambda[0], 0, 35U * sizeof(double));
          int exitg1;
          do {
            exitg1 = 0;
            if (solution->state == -5) {
              ...
              if (subProblemChanged) {
                ...
                switch (activeSetChangeID) {
                 case 1:
                  ix0 = 26 * (workingset->nActiveConstr - 1);
                  ...
                  if (qrmanager->mrows != 0) {
                    ...
                    j = 35 * (qrmanager->mrows - 1) + 1;
                    for (idx = 1; idx <= j; idx += 35) {
                      ix = ix0;
                      c = 0.0;
                  ...
                  for (idx = qrmanager->mrows - 1; idx + 1 > qrmanager->ncols;
                       idx--) {
                    d = qrmanager->QR[idx + 35 * (qrmanager->ncols - 1)];
                    internal::blas::xrotg(&qrmanager->QR[(idx + 35 *
                      (qrmanager->ncols - 1)) - 1], &d, &c, &s);
                    qrmanager->QR[idx + 35 * (qrmanager->ncols - 1)] = d;
                    iyend = 35 * (idx - 1);
                    ia = qrmanager->mrows;
                    ...
                    switch (j) {
                     case 3:
                      ...
                      workingset->Wlocalidx[workingset->nActiveConstr - 1] =
                        iyend;
                      ix0 = 26 * (iyend - 1);
                      iy = 26 * (workingset->nActiveConstr - 1);
                      ia = workingset->nVar;
                      for (ix = 0; ix < ia; ix++) {
              ...
              if (guard2) {
                ...
                if (qrmanager->ncols > 0) {
                  ...
                  if (updateFval) {
                    ...
                    if (b_guard1) {
                      while ((idx >= 1) && (std::abs(qrmanager->QR[(idx + 35 *
                                (idx - 1)) - 1]) >= temp)) {
                        idx--;
                      }
                ...
                if (solution->state != -7) {
                  ...
                  if (ix + 1 == 0) {
                    ...
                    j = workingset->nVar;
                    for (idx = 0; idx < j; idx++) {
                      workingset->ATwset[idx + 26 * ix] = workingset->ATwset[idx
                        + 26 * (workingset->nActiveConstr - 1)];
                    }
