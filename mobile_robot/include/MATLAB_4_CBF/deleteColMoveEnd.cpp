      ...
      namespace QRManager
      {
        void deleteColMoveEnd(g_struct_T *obj, int idx)
        {
          ...
          if (idx >= obj->ncols) {
            ...
            b_i = obj->minRowCol;
            for (k = 0; k < b_i; k++) {
              obj->QR[k + 43 * (idx - 1)] = obj->QR[k + 43 * (obj->ncols - 1)];
            }
            ...
            if (idx < obj->mrows) {
              ...
              for (k = endIdx; k >= idx; k--) {
                b_i = k + 43 * (idx - 1);
                temp = obj->QR[b_i];
                internal::blas::xrotg(&obj->QR[(k + 43 * (idx - 1)) - 1], &temp,
                                      &c, &s);
                obj->QR[b_i] = temp;
                b_ix = 43 * (k - 1);
                obj->QR[k + b_ix] = 0.0;
                i = k + 43 * idx;
                n = obj->ncols - idx;
                std::memcpy(&x[0], &obj->QR[0], 1849U * sizeof(double));
                if (n >= 1) {
                  ix = i - 1;
                  for (b_k = 0; b_k < n; b_k++) {
                    temp = c * x[ix] + s * x[i];
                ...
                std::memcpy(&obj->Q[0], &x[0], 1849U * sizeof(double));
              }
              ...
              for (k = b_i; k <= endIdx; k++) {
                ...
                i = k + b_ix;
                temp = obj->QR[i];
                internal::blas::xrotg(&obj->QR[(k + 43 * (k - 1)) - 1], &temp,
                                      &c, &s);
                obj->QR[i] = temp;
                i = k * 44;
                n = obj->ncols - k;
