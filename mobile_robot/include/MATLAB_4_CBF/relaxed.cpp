  ...
  namespace optim
  {
    namespace coder
    {
      namespace fminconsqp
      {
        namespace step
        {
          void relaxed(const double Hessian[64], const double grad[30],
            ...
            while ((ix + 1 > 0) && (iac > 0)) {
              if ((WorkingSet->Wid[ix] == 4) && (WorkingSet->Wlocalidx[ix] >
                ...
                for (iy = 0; iy < idx_max; iy++) {
                  WorkingSet->ATwset[iy + 30 * ix] = WorkingSet->ATwset[iy + 30 *
                    (WorkingSet->nActiveConstr - 1)];
                }
