  ...
  namespace optim
  {
    namespace coder
    {
      namespace utils
      {
        ...
        {
          int computeConstraints_(MATLAB_OPT_TANK_CBF *aInstancePtr, const
            b_anonymous_function *obj_nonlcon, const double x[8], double
            Cineq_workspace[19])
          {
            double varargout_1[19];
            int idx_current;
            int status;
            boolean_T allFinite;
            aInstancePtr->constraints_cbf(x, obj_nonlcon->tunableEnvironment.f1,
              ...
              obj_nonlcon->tunableEnvironment.f10, varargout_1);
            std::memcpy(&Cineq_workspace[0], &varargout_1[0], 19U * sizeof
                        (double));
            status = 1;
            allFinite = true;
            idx_current = 0;
