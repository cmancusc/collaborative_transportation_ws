        ...
        namespace WorkingSet
        {
            ...
            idx_local)
          {
            ...
            if (0 <= idx_bnd_local - 1) {
              std::memset(&obj->ATwset[i * 28], 0, ((idx_bnd_local + i * 28) - i
                * 28) * sizeof(double));
            }

            obj->ATwset[idx_bnd_local + 28 * (obj->nActiveConstr - 1)] = 2.0 *
              static_cast<double>(TYPE == 5) - 1.0;
            idx_bnd_local += 2;
            i1 = obj->nVar;
            if (idx_bnd_local <= i1) {
              std::memset(&obj->ATwset[(idx_bnd_local + i * 28) + -1], 0, ((((i1
                + i * 28) - idx_bnd_local) - i * 28) + 1) * sizeof(double));
            }
            ...
            switch (obj->probType) {
             ...
             default:
              obj->ATwset[(obj->nVar + 28 * (obj->nActiveConstr - 1)) - 1] =
                -1.0;
              break;
            }
          }
