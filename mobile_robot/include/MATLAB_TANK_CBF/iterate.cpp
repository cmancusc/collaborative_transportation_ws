...
namespace coder
{
  namespace optim
  {
    namespace coder
    {
      ...
      {
        void iterate(const double H[64], const double f[28], d_struct_T
            ...
            if (solution->state == -5) {
              ...
              if (subProblemChanged) {
                ...
                switch (activeSetChangeID) {
                 case 1:
                  ix0 = 28 * (workingset->nActiveConstr - 1);
                  ...
                  if (qrmanager->mrows != 0) {
                    ...
                    j = 39 * (qrmanager->mrows - 1) + 1;
                    for (idx = 1; idx <= j; idx += 39) {
                      ix = ix0;
                      c = 0.0;
                  ...
                  for (idx = qrmanager->mrows - 1; idx + 1 > qrmanager->ncols;
                       idx--) {
                    d = qrmanager->QR[idx + 39 * (qrmanager->ncols - 1)];
                    internal::blas::xrotg(&qrmanager->QR[(idx + 39 *
                      (qrmanager->ncols - 1)) - 1], &d, &c, &s);
                    qrmanager->QR[idx + 39 * (qrmanager->ncols - 1)] = d;
                    iyend = 39 * (idx - 1);
                    ia = qrmanager->mrows;
                    ...
                    switch (j) {
                     case 3:
                      ...
                      workingset->Wlocalidx[workingset->nActiveConstr - 1] =
                        iyend;
                      ix0 = 28 * (iyend - 1);
                      iy = 28 * (workingset->nActiveConstr - 1);
                      ia = workingset->nVar;
                      for (ix = 0; ix < ia; ix++) {
              ...
              if (guard2) {
                ...
                if (qrmanager->ncols > 0) {
                  ...
                  if (updateFval) {
                    ...
                    if (b_guard1) {
                      while ((idx >= 1) && (std::abs(qrmanager->QR[(idx + 39 *
                                (idx - 1)) - 1]) >= temp)) {
                        idx--;
                      }
                ...
                if (solution->state != -7) {
                  ...
                  if (ix + 1 == 0) {
                    ...
                    j = workingset->nVar;
                    for (idx = 0; idx < j; idx++) {
                      workingset->ATwset[idx + 28 * ix] = workingset->ATwset[idx
                        + 28 * (workingset->nActiveConstr - 1)];
                    }
