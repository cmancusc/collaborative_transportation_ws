                            ...
                            *cholmanager, const h_struct_T *objective)
        {
          ...
          if (mNull_tmp <= 0) {
            ...
            if (qrmanager->ncols <= 0) {
              ...
              nullStartIdx_tmp = 39 * qrmanager->ncols + 1;
              switch (objective->objtype) {
               case 5:
                {
                  int idx_col;
                  for (ix = 0; ix <= mNull; ix++) {
                    memspace->workspace_double[ix] = -qrmanager->Q[nVar + 39 *
                      (qrmanager->ncols + ix)];
                  }
                  ...
                  if (qrmanager->mrows != 0) {
                    ...
                    ix = 0;
                    i = nullStartIdx_tmp + 39 * (mNull_tmp - 1);
                    for (iac = nullStartIdx_tmp; iac <= i; iac += 39) {
                      int idx_row;
                      int nVars;
                      idx_row = 0;
                      nVars = iac + idx_col;
                   ...
                   default:
                    ...
                    for (idx_col = 0; idx_col < mNull_tmp; idx_col++) {
                      ...
                      for (idx_row = i; idx_row <= nVars; idx_row++) {
                        memspace->workspace_double[(idx_row + 39 * idx_col) - 1]
                          = objective->beta * qrmanager->Q[(idx_row + 39 *
