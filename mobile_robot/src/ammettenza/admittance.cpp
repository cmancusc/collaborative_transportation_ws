...
// Last modified: 2021/04/24 19:51:28
...
Admittance::Admittance(){
    ...
    odometry_sub = nh.subscribe("/base_pose_ground_truth", 1, &Admittance::OdometryCallback, this);
    joints_state_sub= nh.subscribe("/joint_states", 1, &Admittance::JointStateCallback, this);
    joystick_sub = nh.subscribe("/joy", 1, &Admittance::joystickCallback, this);
...
void Admittance::OdometryCallback(const nav_msgs::Odometry& msg){
...
void Admittance::joystickCallback(const sensor_msgs::Joy& msg){
...
void Admittance::JointStateCallback(const sensor_msgs::JointState::ConstPtr &msg){
...
Matrix4d Admittance::compute_arm_fk(double joint_position[], double joint_velocity[]) {
...
Eigen::MatrixXd Admittance::compute_arm_jacobian (double joint_position[], double joint_velocity[]) {
...
Matrix62d Admittance::compute_platform_jacobian(double rotation)
{
...
Matrix6d Admittance::Arm_Mapping(double rotation){
...
Matrix6d Admittance::Mir_Mapping(double rotation){
...
Matrix6d Admittance::Transformata(double rotation, int num){
...
Vector8d Admittance::limit_joints_dynamics (Vector8d joint_velocity) {
...
Vector8d Admittance::SatNullSpace(Vector6d cartesian_velocity, Eigen::MatrixXd system_jacobian, Vector8d Vel_LowerLimit, Vector8d Vel_UpperLimit){
...
int Admittance::sign (double num) {
...
//############################ SATURATION ######################################//
    ...
    x_dot_opt = jacobian_augmented * q_dot_opt; //! RICORDA DI INSERIRE IL VALORE DI RITORNO DAL TOPIC DI VELOCITA
    ...
    double delta_update = (pow(cycle_time, 2)/ (2 * pow(tank_state_, 2)))*(pow((F_ext).transpose() * x_dot_opt, 2));
