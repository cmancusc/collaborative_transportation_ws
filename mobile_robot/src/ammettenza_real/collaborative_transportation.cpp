...
// Last modified: 2021/05/14 10:08:17
...
System_Controller::System_Controller(){
    ...
    odometry_sub = nh.subscribe("/odom_enc", 1, &System_Controller::OdometryCallback, this);
    joints_state_sub= nh.subscribe("/joint_states", 1, &System_Controller::JointStateCallback, this);
    mir_joint_sub = nh.subscribe("/mir_joint_states", 1, &System_Controller::RuoteCallback, this);
    //joystick_sub = nh.subscribe("/joy", 1, &System_Controller::joystickCallback, this);
    force_sub = nh.subscribe("/wrench", 1, &System_Controller::ForceCallback, this);
    //arm_pub = nh.advertise<trajectory_msgs::JointTrajectory>("/ur10e/manipulator_joint_trajectory_System_Controller/command",1);
    ...
    R = 0.0625;         // wheel radius
    L = 0.445208;       // half length of wheel axis
    cycle_time = 0.02;
...
//    omega_r = 0.0;
    ...
    b_ = -0.7;          //IO-SFL lead value

    measure_hat_8.setZero();
    P_8.setZero();
    K_8.setZero();
...
//   }
    ...
    //######## Define the fixed, min, max value of admittance parameters #######//
    ...
    file_path << "/home/federico/MATLAB_ws/RAL21-Collaborative_Transportation/experiments_ICRA/q_dot_scaling_debug.txt";
    q_dot_file.open(file_path.str());

    std::stringstream file_path1;
    file_path1 << "/home/federico/MATLAB_ws/RAL21-Collaborative_Transportation/experiments_ICRA/q_dot_SNS_debug.txt";
    q_dot_SNS_file.open(file_path1.str());

    std::stringstream file_path2;
    file_path2 << "/home/federico/MATLAB_ws/RAL21-Collaborative_Transportation/experiments_ICRA/force_debug.txt";
    force_file.open(file_path2.str());

    std::stringstream file_path3;
    file_path3 << "/home/federico/MATLAB_ws/RAL21-Collaborative_Transportation/experiments_ICRA/q_dot_opt_debug.txt";
    q_dot_opt_file.open(file_path3.str());

    std::stringstream file_path4;
    file_path4 << "/home/federico/MATLAB_ws/RAL21-Collaborative_Transportation/experiments_ICRA/energy_tank_debug.txt";
    tank_file.open(file_path4.str());

    std::stringstream file_path5;
    file_path5 << "/home/federico/MATLAB_ws/RAL21-Collaborative_Transportation/experiments_ICRA/q_dot_opt_filtered_file.txt";
    q_dot_opt_filtered_file.open(file_path5.str());

    std::stringstream file_path7;
    file_path7 << "/home/federico/MATLAB_ws/RAL21-Collaborative_Transportation/experiments_ICRA/parametri_ammettenza_file.txt";
    parametri_ammettenza.open(file_path7.str());
...
System_Controller::~System_Controller(){
    ...
    q_dot_opt_filtered_file.close();
    tank_file.close();
    parametri_ammettenza.close();

}


void System_Controller::OdometryCallback(const nav_msgs::Odometry& msg){
...
void System_Controller::RuoteCallback(const sensor_msgs::JointState& msg){
...
//void System_Controller::joystickCallback(const sensor_msgs::Joy& msg){
...
void System_Controller::JointStateCallback(const sensor_msgs::JointState::ConstPtr &msg){
...
void System_Controller::ForceCallback(const geometry_msgs::WrenchStamped::ConstPtr &msg){
...
*/
    ...
    for(int i = 3; i < F_ext.rows(); i++){
        if((F_ext(i) > 0.0) && (F_ext(i) < 0.2))
            F_ext(i) = 0.0;

        else if((F_ext(i) < 0.0) && (F_ext(i) > - 0.2))
            F_ext(i) = 0.0;
    }
...
Matrix4d System_Controller::compute_arm_fk(double joint_position[], double joint_velocity[]) {
...
Vector6d System_Controller::compute_ee_pose(double joint_position[], double joint_velocity[]) {
...
//    ee_pose << translation_vector, roll_ee, pitch_ee, yaw_ee;
    ee_pose << translation_vector, 0.0, 0.0, 0.0;
    ee_pose(2) = 0.0;
...
Eigen::MatrixXd System_Controller::compute_arm_jacobian (double joint_position[], double joint_velocity[]) {
...
Matrix62d System_Controller::compute_platform_jacobian(double rotation){
...
Matrix6d System_Controller::Arm_Mapping(double rotation){
    ...
    // orientation angle between UR_base frame and MiR frame
  ...
  //---------------------------------------------------------------------------------------------------//
    ...
    // quindi dovrei avere velocità di EE rspetto a World
...
// inutile
Matrix6d System_Controller::Mir_Mapping(double rotation){
...
// inutilizzata
Matrix6d System_Controller::Transformata(){
...
Matrix6d System_Controller::get_ee_rotation_matrix(double joint_position[], double joint_velocity[]) {
...
Vector6d System_Controller::Force_in_Odom_Frame(){
...
// limitazione velicità e accelerazione utilizzando lo scaling
// attualmente utilizzato solo come confronto ma non partecipa alla computazione del sistema
Vector8d System_Controller::limit_joints_dynamics (Vector8d joint_velocity) {
...
Vector8d System_Controller::SatNullSpace(Vector6d cartesian_velocity, Eigen::MatrixXd system_jacobian, Vector8d Vel_LowerLimit, Vector8d Vel_UpperLimit){
...
void System_Controller::send_velocity_to_robot (Vector6d velocity) {
...
int System_Controller::sign (double num) {
...
Vector8d System_Controller::KALMAN_8(Vector8d measure){
...
//    Vector8d K; //kalman gain
    ...
    static const double R = 400; // (10 .. 0.33) noise covariance
    static const double H = 1.0; //measurement map scalar
    static double Q = 5; // (15) initial estimated covariance
//    Vector8d P; //initial error covariance
//    Vector8d measure_hat; //initial estimated state
...
//    Vector8d K; //kalman gain
    ...
    for(int i = 0; i < measure.rows(); i++){
        K_8(i) = P_8(i) * H / (H * P_8(i) * H + R); // update the gain
        measure_hat_8(i) += K_8(i) * (measure(i) - H * measure_hat_8(i)); //update the estimation
        P_8(i) = (1 - K_8(i) * H) * P_8(i) + Q;
    }


    estimated = measure_hat_8;
    return estimated;
}



void System_Controller::Spinner()
{
...
//    for (int i = 2; i < end_effector_pose.rows(); i++)
//        end_effector_pose(i) = 0.0;
...
//        x[i] = end_effector_pose(i);
    ...
    Vector6d ee_actual_pose; //actual pose in odom frame only x and y
    for(int i = 0; i < ee_actual_pose.rows(); i++){
      if(i < 2){
        ee_actual_pose(i) = end_effector_pose(i) + ur_base_world_pose(i);
      }
      else{
        ee_actual_pose(i) = 0.0;
      }
    }
...
//    double alpha_h1, alpha_h2, alpha_h2_up, alpha_h2_dx;
...
//    MATLAB_OPT.matlab_opt_4_cbf(adm_qdot_matlab, q_dot_0, A_matlab, B, x, x_0, x_up, x_dx, alpha_h1, alpha_h2,
//                                alpha_h2_up, alpha_h2_dx, Jacob, Jacob_limit, a_max, opt_qdot_prev_matlab,
//                                opt_qdot_matlab, &costfun_val, &exit_flag);
...
//    }
    ...
    double future_q_man[6]; //future_q_manipulator
    double actual_q_dot_man[6];
    Vector6d ee_future_pose;
    bool mi_muovo = true;
    double z_limit = 0.5;  // raggio cilinfro virtuale

    // posizione dei giunti futuri se pubblicassi le velocità ottimizzate
    for (int i = 0; i < 6; ++i) {
        future_q_man[i] = joint_real_position[i] + q_dot_opt(i) * cycle_time;
        actual_q_dot_man[i] = q_dot_opt(i); //vector to double
    }

    // future cartesian pose of ee in odom frame
    ee_future_pose = compute_ee_pose(future_q_man, actual_q_dot_man);

    for(int i = 0; i < ee_future_pose.rows(); i++){
      if(i < 2){
        ee_future_pose(i) += ur_base_world_pose(i);
      }
      else{
        ee_actual_pose(i) = 0.0;
      }
    }
    ROS_INFO_STREAM("EE FUTURE POSE: \t" << ee_future_pose);
    ROS_INFO_STREAM("EE ACTUAL POSE: \t" << ee_actual_pose);


    if((ee_future_pose.norm() > z_limit) && (ee_future_pose.norm() > ee_actual_pose.norm())){
      mi_muovo = false;
    }
    ...
    if(mi_muovo == false){
      for (int i = 0; i < 8; i++) {
        if(i < 6){
          adm_qdot_matlab[i] = 0.0;
        }
        else{
          adm_qdot_matlab[i] = adm_qdot_matlab[i];
        }
      }
  ...
  //############################## SAVE DATA ####################################//
    ...
    q_dot_opt_filtered_file << time_now;
    tank_file << time_now;
    parametri_ammettenza << time_now;
    ...
    // ##############################################################
    // filtre le velocità dei giunti con KALMAN
    // ##############################################################
    Vector8d q_dot_filtered;
    q_dot_filtered = KALMAN_8(q_dot_opt);


    double error_pos = -ee_actual_pose.norm() + 0.5;

    for(int i = 0; i < 8; i++)
        q_dot_opt_filtered_file << " " << q_dot_filtered(i);

    q_dot_opt_filtered_file  << " " << error_pos;

    double error = (q_dot_opt - adm_q_dot_scaling).norm();


    tank_file << " " << tank_state_ << " " << tank_energy_ << " " << error;

/*
    double norma_massa = variable_mass.norm();
    double norma_damping = variable_damping.norm();

    for(int i = 0; i < 6; i++)
        parametri_ammettenza << " " << variable_mass(i);


    for(int i = 0; i < 6; i++)
        parametri_ammettenza << " " << variable_damping(i);


    parametri_ammettenza << " " << norma_massa << " " << norma_damping;
*/
    ...
    q_dot_opt_filtered_file << std::endl;
    tank_file << std::endl;
    parametri_ammettenza << std::endl;
...
//    }
    ...
    for(int i = 0; i < adm_qdot_arm.rows(); i++){
      adm_qdot_arm(i) = q_dot_filtered(i);
    }
...
//    }
    ...
    for(int i = 6; i < q_dot_filtered.rows(); i++){
      adm_qdot_base(i-6) = q_dot_filtered(i);
    }
...
//    ROS_INFO_STREAM("h_2: \t" << h_2);
    ROS_INFO_STREAM("Prova Compilazione \t 0");
    ROS_INFO_STREAM("MI MUOVO: \t" << mi_muovo);
    ROS_INFO_STREAM("EE FUTURE POSE: \t" << ee_future_pose);
    ROS_INFO_STREAM("EE ACTUAL POSE: \t" << ee_actual_pose);
